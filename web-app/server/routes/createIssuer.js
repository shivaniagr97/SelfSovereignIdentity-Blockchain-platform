...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const userExists = await wallet.exists(req.body.issuerID);
        if (userExists) {
        ...
        const secret = await ca.register({
            ...
            enrollmentID: req.body.issuerID,
            role: 'client'
        }, adminIdentity);
        const enrollment = await ca.enroll({enrollmentID: req.body.issuerID, enrollmentSecret: secret});

        // console.log(JSON.parse(enrollment.toString()));

        const userIdentity = X509WalletMixin.createIdentity('Org1MSP', enrollment.certificate, enrollment.key.toBytes());

        await wallet.import(req.body.issuerID, userIdentity);
        ...
        if (response.length === 73) {
            const enrollment = await ca.revoke({enrollmentID: req.body.issuerID}, adminIdentity);
            await wallet.delete(req.body.issuerID);
            res.send(response);
        } else {
            res.send("Correct");
        }
