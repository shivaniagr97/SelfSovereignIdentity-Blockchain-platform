...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const userExists = await wallet.exists(req.body.verifierID);
        if (userExists) {
        ...
        const secret = await ca.register({
            ...
            enrollmentID: req.body.verifierID,
            role: 'client'
        }, adminIdentity);
        const enrollment = await ca.enroll({enrollmentID: req.body.verifierID, enrollmentSecret: secret});

        // console.log(JSON.parse(enrollment.toString()));

        const userIdentity = X509WalletMixin.createIdentity('Org1MSP', enrollment.certificate, enrollment.key.toBytes());

        await wallet.import(req.body.verifierID, userIdentity);
        ...
        if (response.length === 73) {
            const enrollment = await ca.revoke({enrollmentID: req.body.verifierID}, adminIdentity);
            await wallet.delete(req.body.verifierID);
            res.send(response);
        } else {
            res.send("Correct");
        }
...
async function registerInLedger(req) {
    ...
    try {
        ...
        let docTypes = [];
        docTypes.push(req.body.docTypes);
        req.body.docTypes = docTypes;
        let response = await contract.submitTransaction('createVerifier', JSON.stringify(req.body));
